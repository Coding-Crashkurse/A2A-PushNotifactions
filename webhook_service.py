...
async def startup_event():
    ...
    logger.info(
        f"🚀 Webhook Service started. Listening on http://{WEBHOOK_HOST}:{WEBHOOK_PORT}"
    )
    logger.info(
        f"   Task logs will be saved in '{os.path.abspath(RESULTS_DIR)}/' directory."
    )
...
async def receive_notification(
    ...
    logger.info(
        f"🔔 INCOMING NOTIFICATION for Task ID: {task.id} from {request.client.host}"
    )
    try:
        ...
        logger.error(
            f"🚨 Unhandled error while processing notification for Task {task.id}: {e}\n{traceback.format_exc()}"
        )
