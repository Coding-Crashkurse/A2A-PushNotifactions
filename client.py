        ...
        async with httpx.AsyncClient(timeout=60.0) as http_client:
            ...
            while True:
                ...
                push_config = PushNotificationConfig(
                    url=WEBHOOK_URL, token=NOTIFICATION_TOKEN
                )
                send_config = MessageSendConfiguration(
                ...
                request = SendMessageRequest(
                    params=MessageSendParams(
                        message=user_message, configuration=send_config
                    ),
                    id=request_id,
                )
                ...
                if isinstance(task_object, Task):
                    logger.info(
                        f"✅ Request Acknowledged. Task ID: {task_object.id}. Agent is working."
                    )
                    logger.info("   The result will be sent to the webhook service.")
                else:
                    logger.error(
                        f"🚨 Received an unexpected response. Content: {response}"
                    )
